...
export function HeroBanner({ banner }: { banner: Banner }) {
  return (
    <section className="container py-4 md:py-7">
      <div className="section-surface relative overflow-hidden">
        ...
        <div className="relative grid min-h-[auto] lg:min-h-[520px] lg:grid-cols-[0.9fr_1.1fr]">
          <div className="relative z-10 flex items-center px-4 py-6 md:px-8 md:py-10 lg:px-12">
            <div className="max-w-xl">
              ...
              <h1 className="text-balance text-[2rem] font-black leading-[1.05] text-[var(--brand-dark)] md:text-5xl lg:text-6xl">
                {banner.title}
              </h1>
              <p className="mt-3 max-w-xl text-sm leading-6 text-[var(--muted)] md:mt-4 md:text-lg md:leading-8">{banner.subtitle}</p>
              <div className="mt-6 flex flex-wrap gap-2.5 md:mt-7 md:gap-3">
                <Link
                  className="inline-flex items-center gap-2 rounded-full bg-[var(--accent)] px-5 py-3 text-sm font-bold text-black shadow-[0_10px_25px_rgba(255,171,16,0.3)] md:px-6 md:text-base"
                  href={banner.href}
                >
                  Start shopping
                  <ArrowRight size={17} />
                </Link>
                <Link className="rounded-full border border-[#cbe8bd] bg-white px-5 py-3 text-sm font-bold text-[var(--brand)] md:px-6 md:text-base" href="/shop/fresh-vegetables">
                  Fresh vegetables
                </Link>
              </div>
              <div className="mt-5 grid gap-2.5 md:mt-7 md:gap-3 sm:grid-cols-3">
