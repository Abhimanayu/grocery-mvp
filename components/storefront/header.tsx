      ...
      <header className="sticky top-0 z-40 border-b border-[var(--border)] bg-white/95 shadow-[0_10px_35px_rgba(57,64,74,0.08)] backdrop-blur">
        <div className="hidden bg-[var(--brand-dark)] py-2 text-sm text-white md:block">
          <div className="container flex items-center justify-between gap-4">
            <span>Fresh fruits & vegetables delivered across Jaipur</span>
            <span className="font-semibold">{siteConfig.phone} · {siteConfig.email}</span>
          </div>
        </div>
        <div className="container flex h-[76px] items-center gap-2 md:gap-3">
          ...
          <button className="focus-ring hidden min-w-[198px] items-center gap-2 rounded-full border border-[#d7edcc] bg-[#f8fffa] px-4 py-2 text-left text-sm lg:flex">
          ...
          <form className="relative min-w-0 flex-1" onSubmit={onSearch}>
            ...
            <input
              className="focus-ring h-12 w-full rounded-full border border-[#d7edcc] bg-[#f8fffa] pl-10 pr-4 text-sm shadow-inner"
              placeholder="Search"
              value={query}
          ...
          <button
            className="focus-ring relative grid size-12 place-items-center rounded-full bg-[var(--brand)] text-white shadow-[0_12px_24px_rgba(101,176,27,0.25)]"
